...
class BSplineCurve:
    ...
    def getNormalized(self,rotate=True,diag=True,eps=1e-2):
...
def drawBSplineCurve3D(curve:BSplineCurve,sample_size=64,show_ctrl_points=False,scatter=False,view='xyz'):
    points=samplePointsFromCurve(curve,max_point_length=sample_size,parameter_generator=ConstantStepParameterGenerator)
    points=np.array(points)
    fig, ax = plt.subplots(subplot_kw=dict(projection='3d'))
    ...
    if scatter:
        ...
        ax.plot(points[:,x],points[:,y],points[:,z],c='b')
...
def main():
    ...
    # crv=generator.genCurves(num=1)[0]
    
    crv=BSplineCurve(degree=3,control_pts=np.array([[0,0],[1,0],[1,1],[0,1]]),knot_vector=np.array([0,0,0,0,1,1,1,1]))
    
    drawBSplineCurve(curve=crv,draw_ctrl_label=False,draw_ctrl_lines=False,sample_size=1000)
    plt.show()
